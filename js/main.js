...
function adaptive_header(w,h) {
	...
	if(w<767){
		if(!$('.header-botton-menu').hasClass('done')){
			$('.header-botton-menu').addClass('done').appendTo(headerMenu);
		}
	}else{
		$.each($('.header-botton-menu'), function(index, val) {
			if($(this).hasClass('header-botton-menu--right')){
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.hheader-botton__column').eq(2));
				}
			}else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.hheader-botton__column').eq(0));
				}
			}
		});
	}
